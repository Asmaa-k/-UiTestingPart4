...
import com.example.espressouitestfragment.ui.util.EspressoIdlingResourceRule
...
class MovieListFragmentTest {
    ...
    fun test_navStarActorsFragment_validateActorsList() {
        ...
        // Nav to ActorsFragment
        onView(withId(R.id.movie_star_actors)).perform(click())

        // Confirm correct directors are visible
        onView(withId(R.id.star_actors_text)).check(
            matches(withText(StarActorsFragment.stringBuilderForStarActors(MOVIE_IN_TEST.star_actors!!)))
        )
    }
}